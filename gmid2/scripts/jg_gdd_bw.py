...
from gmid2.inference.bucket import mini_bucket_tree_decomposition, join_graph_decomposition
from gmid2.inference.pgm_gdd import PgmGDD


def run(file_path, ibound):
    print("{}\t\t{}".format(PgmGDD.__name__, ibound))
    ...
    jg = join_graph_decomposition(mbt, bw_ordering)
    iter_options = { 'iter_limit':100, 'cutoff_diff': 1e-3}
    opt_options = { 'ls_steps': 30, 'armijo_thr': 1e-4, 'armijo_step_back': 0.5,
                    'ls_tolerance':TOL, 'gd_steps':20, 'tolerance': TOL}
    gdd = PgmGDD(gm, bw_ordering, iter_options, opt_options)
